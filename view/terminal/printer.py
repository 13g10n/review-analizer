...
class TerminalPrinter:
    ...
    def __get_text_report_string(report):
        return TEXT_REPORT.format(
            TITLE=report.get_excerpt(),
            SENTENCES_COUNT=report.sentences_count,
            POSITIVE=report.positive,
            NEGATIVE=report.negative,
            SUMMARY=report.summary,
            SENTENCE_REPORTS="".join([TerminalPrinter.__get_sentence_report_string(report) for report in report.get_sentences()])
        )
    ...
    def __get_sentence_report_string(report):
        return SENTENCE_REPORT.format(
            ...
            KEYWORD_REPORT="".join([TerminalPrinter.__get_keyword_report_string(report) for report in report.get_reports()])
        )
